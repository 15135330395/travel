	...
	<div class="x-body">
		...
		<div style="height: 50%">
			<table class="layui-table">
				...
				<tbody>
					<c:forEach items="${queryOrderBySid}" var="list">
						<tr>
							<td>${list.orderId}</td>
		...
		<div class="page">
			<div>
				<c:if test="${pageBean.index>1}">
					<a class="prev"
						href="<%=request.getContextPath()%>/QueryOrderController/queryBySid?index=${pageBean.index-1}">&lt;&lt;</a>
				</c:if>
				<c:if test="${pageBean.index<=1}">
					<a class="prev"
						href="<%=request.getContextPath()%>/QueryOrderController/queryBySid?index=${pageBean.index}">&lt;&lt;</a>
				</c:if>
				<c:forEach var="i" begin="1" end="${pageBean.pages}" step="1">
					...
					<c:if test="${i!=pageBean.index}">
						<a class="num"
							href="<%=request.getContextPath()%>/QueryOrderController/queryBySid?index=${i}">
							${i} </a>
					</c:if>
				</c:forEach>
